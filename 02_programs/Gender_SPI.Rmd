...
table1 <- disagg_tab_df %>%
  select(Goal, Indicator, Tier, Description, `Sex Disaggregation Available`, `Any Data Available`) %>%
  merge(table1dat, by = c("Goal", "Indicator", "Tier", "Description"), all = T) %>%
...
SPI_df <- wbstats::wb_data(indicator=c('IQ.SPI.OVRL', 'NY.GDP.PCAP.PP.CD'),
                           start_date = 2019)
...
top_outcome<-round(gender_overperformers_df$outcome[1],1)
top_pred<-round(gender_overperformers_df$pred[1],1)
top_res<-round(gender_overperformers_df$res[1],1)
