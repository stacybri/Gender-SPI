...
all_sdg_df <- read_csv(paste0(output_dir, "/SPI_D3_UNSD_alltier_data_5yr.csv")) %>%
...
# get totals for all SDGs
all_gender_sdg_df <- read_csv(paste0(output_dir, "/SPI_D3_UNSD_gender_data_5yr.csv")) %>%
  filter(date==upper_year) %>%
  filter(!(iso3c %in% pop_exlude_cntry)) %>%
    #collapse to country level
...
# get totals for all SDGs
all_nongender_sdg_df <- read_csv(paste0(output_dir, "/SPI_D3_UNSD_nongender_data_5yr.csv")) %>%
  filter(date==upper_year) %>%
  filter(!(iso3c %in% pop_exlude_cntry)) %>%
    #collapse to country level
  group_by(iso3c, country, goal) %>%
