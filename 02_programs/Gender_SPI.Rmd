...
write_excel_csv(un_aki, path=paste(output_dir, 'SPI_Gender_UNSD_data_3yr.csv', sep="/"))
...
#create database of gender indicators whether they exist at all
  ...
  sdg_gender_exists_df <- un_sdg_gender_df %>%
    filter(between(date,lower_year,upper_year) ) %>%
    filter(!is.na(ind_quality)) %>%
...
disagg_plt_df <- sdg_gender_disagg_df %>%
  ...
  left_join(list_df %>% select(code, description1, goal)) %>%
...
disagg_tab_df <- sdg_gender_disagg_df %>%
  ...
  left_join(list_df %>% select(code, description, goal, tier)) %>%
  filter(sex %in% c("FEMALE", "Any")) %>%
  mutate(sex=if_else(sex=="FEMALE","Gender Breakdown Available",'Any Data Available'),
...
disagg_tab_df %>%
  select(Goal, Indicator, Tier, Description, `Gender Breakdown Available`, `Any Data Available`) %>%
  flextable() %>%
    add_header_lines('Table 2. Gender Breakdowns Available for SDG Indicators') %>%
    merge_v(j='Goal') %>%
    set_formatter( `Gender Breakdown Available` = function(x) sprintf( "%.1f%%", x ) ) %>%
...
SPI_df <- wbstats::wb_data(indicator='IQ.SPI.OVRL',
                           start_date = 2019)
...
gend_spi_plt <-combined_df %>%
  ggplot(aes(y=IQ.SPI.OVRL, x=availability)) +
  geom_text(aes(label=iso3c)) +
  geom_smooth(method='lm') +
  theme_spi() +
  ylab('SPI Overall Score') +
  xlab('Availability of Gender SDG Indicators (%)') +
  geom_richtext(
    aes(x = 75, y = 5,label = eq_plot_txt(combined_df), hjust=0.2)
  ) +  
  theme(legend.position = 'bottom') +
  expand_limits(x=c(0,100),
                y=c(0,100)) +
  labs(
    title='Plot of SPI overall score on Availability of Gender SDG Indicators',
    caption=str_wrap('Source: SPI Overall Score from the World Development Indicatrs (IQ.SPI.OVRL).  The availability of Gender SDG Indicators based on authors calculations based on the UN Global SDG Database.',100)
  )
...
sources_explained <-sources_categorized_df %>%
  group_by(data_type, code, description, description1) %>%
  summarise(n=sum(n),
