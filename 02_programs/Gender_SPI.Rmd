...
avail_plt_df5 <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
...
avail_SPI_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
  filter(date==2020 & goal=="Combined") #keep just most recent year
...
gend_gdp_plt <-combined_df %>%
  ...
  ylab('Availability of Gender SDG Indicators (%)') +
  geom_richtext(
    aes(y = 87, x = 40000,label = eq_plot_txt(combined_df, availability, log10(NY.GDP.PCAP.PP.CD)), hjust=0.2)
  ) +  
  theme(legend.position = 'bottom') +
  expand_limits(y=c(0,100)) +
  labs(
...
gend_gdp_plt2 <- avail_wbstats_df %>%
  ...
  ylab('Availability of Gender SDG Indicators (%)') +
  geom_richtext(
    aes(y = 87, x = 40000,label = eq_plot_txt(avail_wbstats_df, availability, log10(NY.GDP.PCAP.PP.CD)), hjust=0.2)
  ) +  
  theme(legend.position = 'bottom') +
  expand_limits(y=c(0,100)) +
  labs(
...
```{r}
#get overall gender data availability measure
avail_full_SPI_df <- read_csv(paste0(output_dir, '/SPI_Gender_UNSD_data_5yr.csv')) %>%
  mutate(availability=100*ind_quality) %>% #convert ind_quality measure which is 0-1 to percentage available by multiplying by 100
...
combined_full_df <- avail_full_wbstats_df %>%
  filter(!is.na(IQ.SPI.OVRL)) %>%
  mutate(`Combined Indicators (n=50)`=Combined*50/100,
         `Tier 1 Indicators (n=18)`=`Tier 1`*18/100,
         `Tier 2 Indicators (n=31)`=`Tier 2`*31/100)
...
indicators <- c("Combined Indicators (n=50)", "Tier 1 Indicators (n=18)", "Tier 2 Indicators (n=31)")
reference_year <- 2019
...
mics_dhs_share <- sources_categorized_df %>%
  left_join(list_df, by=c("goal", "target", "code", "code1", 'description', 'description1')) %>%
  group_by(ccode) %>%
  filter(!is.na(ccode)) %>%
  mutate(total=sum(n),
         number=n) %>%
  ...
  group_by(ccode) %>%
  summarise(total_mics_dhs=sum(number)/mean(total)) %>%
