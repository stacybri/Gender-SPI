...
To begin, a discussion of the scope of missing data is given.  Data is largely sourced from the UGepecific breakdown by sex.  
...
```{r save, echo=FALSE, message=FALSE, warning=FALSE, show_col_types = FALSE}
...
```{r readgender, echo=FALSE, show_col_types=FALSE}
...
Figure 1. Percentage of SDG Gender Indicators available between `r 2020-window+1` and 2020.  Each point corresponds to country
```{r avail_chart1}
...
Figure 1. Percentage of SDG Gender Indicators available between `r 2020-window+1` and 2020.  Each point corresponds to country By income group
```{r avail_chart11}
...
Figure 2. Percentage of SDG Gender Tier 1 Indicators available between `r 2020-window+1` and 2020.  Each point corresponds to country
```{r avail_chart21}
...
Figure 2. Percentage of SDG Gender Tier 1 Indicators available between `r 2020-window+1` and 2020.  Each point corresponds to country by income group
```{r avail_chart22}
...
Figure 3. Percentage of SDG Gender Tier 2 Indicators available between `r 2020-window+1` and 2020.  Each point corresponds to country
```{r avail_chart2}
...
Figure 3. Percentage of SDG Gender Tier 2 Indicators available between `r 2020-window+1` and 2020.  Each point corresponds to country by income group
```{r avail_chart3}
...
Figure 4. Percentage of SDG 5 (Gender Equity) Indicators available between `r 2020-window+1` and 2020.  Each point corresponds to country
...
combined_df_sv <- combined_df %>%
  select(iso3c, country, region, IQ.SPI.OVRL, availability, IQ.SPI.PIL3, NY.GDP.PCAP.PP.CD, SP.POP.TOTL) %>%
...
combined_df2 <- avail_SPI_df %>%
  ...
  mutate(outcome=log10(NY.GDP.PCAP.PP.CD),
         value=availability)
...
#add equations to plots
eq_plot_txt_cust <- function(data) {
  eq <- lm_robust(availability ~ IQ.SPI.PIL3, data=data, se_type='HC2')
  coef <- coef(eq)
  std_err <- sqrt(diag(vcov(eq)))
  r_2<- summary(eq)$r.squared
  sprintf("y = %.3f + %.3f x, R<sup>2</sup> = %.3f <br>    (%.3f)   (%.3f)", coef[1], coef[2], r_2[1], std_err[1], std_err[2])
...
gend_spi_plt <-combined_df %>%
  ...
  ylab('Availability of Gender SDG Indicators (%)') +
  geom_richtext(
    aes(x = 75, y = 5,label = eq_plot_txt_cust(combined_df), hjust=0.2)
  ) +  
  theme(legend.position = 'bottom') +
  expand_limits(x=c(0,100),
                y=c(0,60)) +
...
#add equations to plots
eq_plot_txt_cust <- function(data) {
  eq <- lm_robust(IQ.SPI.OVRL ~ log10(NY.GDP.PCAP.PP.CD), data=data, se_type='HC2')
  coef <- coef(eq)
  std_err <- sqrt(diag(vcov(eq)))
  r_2<- summary(eq)$r.squared
  sprintf("y = %.3f + %.3f x, R<sup>2</sup> = %.3f <br>    (%.3f)   (%.3f)", coef[1], coef[2], r_2[1], std_err[1], std_err[2])
...
spi_gdp_plt <-combined_df2 %>%
  ...
  geom_richtext(
    aes(y = 87, x = 40000,label = eq_plot_txt_cust(combined_df2), hjust=0.2)
  ) +  
  theme(legend.position = 'bottom') +
  expand_limits(y=c(0,100)) +
  labs(
